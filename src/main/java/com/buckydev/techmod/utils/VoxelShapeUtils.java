...
/**
 * Copied from Mekanism, with added extras
 * <p>
 * <a href="https://github.com/mekanism/Mekanism/blob/432e917ed6694ccd77a3fdffbdf98c357c58cd87/src/main/java/mekanism/common/util/VoxelShapeUtils.java">Mekansim {VoxelShapeUtils}</a>
 */
public final class VoxelShapeUtils {
    ...
    public static void printSimplified(String name, VoxelShape shape) {
        TechMod.LOGGER.info("Simplified: {}", name);
    ...
    public static VoxelShape rotate(VoxelShape shape, UnaryOperator<AABB> rotateFunction) {
        ...
        for (AABB sourceBoundingBox : sourceBoundingBoxes) {
            ...
            rotatedPieces.add(Shapes.create(rotateFunction
                    .apply(sourceBoundingBox.move(fromOrigin.x, fromOrigin.y, fromOrigin.z))
    ...
    public static <DATA> VoxelShape rotate(VoxelShape shape, DATA data, BiFunction<AABB, DATA, AABB> rotateFunction) {
        ...
        for (AABB sourceBoundingBox : sourceBoundingBoxes) {
            // Make the bounding box be centered around the middle, and then move it back after rotating
            rotatedPieces.add(Shapes.create(rotateFunction
                    .apply(sourceBoundingBox.move(fromOrigin.x, fromOrigin.y, fromOrigin.z), data)
                    .move(-fromOrigin.x, -fromOrigin.z, -fromOrigin.z)));
        }
        // return the recombined rotated voxel shape
        return combine(rotatedPieces);
    ...
    public static VoxelShape batchCombine(
            VoxelShape initial, BooleanOp function, boolean simplify, Collection<VoxelShape> shapes) {
     ...
     */
    public static VoxelShape batchCombine(
            VoxelShape initial, BooleanOp function, boolean simplify, VoxelShape... shapes) {
        VoxelShape combinedShape = initial;
        for (VoxelShape shape : shapes) {
            combinedShape = Shapes.joinUnoptimized(combinedShape, shape, function);
        }
    ...
    public static Map<Direction, VoxelShape> horizontalMap(VoxelShape shape) {
        return directionMap(shape, HORIZONTAL_DIRECTIONS, VoxelShapeUtils::rotateHorizontal);
    }

    public static Map<Direction, VoxelShape> directionMap(VoxelShape shape) {
        return directionMap(shape, DIRECTIONS, VoxelShapeUtils::rotate);
    }

    public static Map<Direction, VoxelShape> directionMap(
            VoxelShape shape, Direction[] directions, BiFunction<VoxelShape, Direction, VoxelShape> func) {
        Map<Direction, VoxelShape> map = new HashMap<>();
        for (Direction dir : directions) {
            map.put(dir, func.apply(shape, dir));
        }
        return map;
    }
}
